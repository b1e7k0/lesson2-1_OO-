...
# -> str = это показывает нам каким типом данных должен будет к нам вернуться функция
# def - это фунция
# return - это функция в def которая показывает нам ким образом функция должна к нам вернуться
# (т.е. показывает что нам должно вывести переменная)
# *, day: int, month: str  =  здесь мы указываем элементы внутри переменной и каким типом данных они должны быть
# (прописывать типы данных и "*", не обязательно)
# isnumeris() - проверяет состоит ли элимент из чисел
# is - группа методов определяющая из чего состоит строка